...
export default function(opt, canvasTexture) {
  ...
  materialRTT = new THREE.ShaderMaterial( {
  	uniforms: { "time": { type: "f", value: 0.0 },   "resolution"  : { type: "v2", value: new THREE.Vector2(window.innerWidth, window.innerHeight) } },
  ...
  function fboInit() {
    ...
    fbShaders = [
        ...
        toolbeltShadersB.paint,
        toolbeltShadersB.warp,
        toolbeltShaders.paint
    ];
  ...
  function fboDraw () {
    ...
    fbMaterial.fboCollection.forEach(function (fbo) {
      ...
      // fbo.mat.uniforms.mouse.value = new THREE.Vector2(Math.sin(noise.getVal(time)*noise.getVal(time)) *2.0, Math.cos(noise.getVal(time)*noise.getVal(time))*2.0);
